...
Options:
    ...
    -ncbidir     <str>  : set the path which contains the program blastpgp 
    -blastdb     <str>  : set the blast database
...
by Nanjiang Shu, created 2010-09-16, updated 2016-02-24
...
sub MyScanDG #{{{
{
    ...
    if ($tmp_informat eq "prf" )
    {
        ...
        use File::Temp;
        my $tmpdir = File::Temp::tempdir("/tmp/myscanDG.XXXXXXXX", CLEANUP=>0);
        read_modhmm_prf($IN,$tmpdir, \%query);
        close($IN);
        ...
        if ($isClean == 1){
            ...
            print "\ntemporary files are kept at $tmpdir\n";
        }
    }
...
sub read_modhmm_prf  #{{{
{
    my ($seq_or_fh, $tmpdir, $profile)=@_;
    my $fh;
    if ($seq_or_fh =~ /^[A-Z]+$/) {     # Seems like a sequence
	my $seq=$seq_or_fh;
	open(OUT,">$tmpdir/DGpred_tmpseq.fasta.2");
	print OUT ">temp\n$seq\n";
	close(OUT);
	open($fh,"$tmpdir/DGpred_tmpseq.fasta.2");
	...
	if (/^>(.+)$/) {    # Looks like a FASTA file!
	    ...
	    chomp($seq);
	    open(OUT,">$tmpdir/DGpred_tmpseq.fasta") || die("Could not open file: $tmpdir/DGpred_tmpseq.fasta");
	    print OUT ">$name\n$seq\n";
	    close(OUT);
	    `$ncbi_dir/bin/blastpgp -a 4 -j 1 -h 1e-5 -e 1e-5 -b 100 -v 0 -d $blastdb -m 6 -i $tmpdir/DGpred_tmpseq.fasta > $tmpdir/DGpred_tmpseq.blast`;
	    `$scampi_msa_bin/msa62mod_oneround.pl $tmpdir/DGpred_tmpseq.blast $tmpdir/DGpred_tmpseq.fasta > $tmpdir/DGpred_tmpseq.prf`;
	    close($fh);
	    open($fh,"$tmpdir/DGpred_tmpseq.prf");
	    return read_modhmm_prf($fh,$tmpdir, $profile);     # Recursive
	} else {
	    /^ALPHABET/ && last;
	}
    }
